...
void setup() 
{
  ...
  pinMode(AUTON_BTN, INPUT_PULLUP);
...
void loop() {
  // check speed autonomous
  bool currState = digitalRead(AUTON_BTN);
  ...
  data.forward  = mapJoystickValues( analogRead(A2), 0, 512, 1024, true );
  data.turn     = mapJoystickValues( analogRead(A1), 0, 512, 1024, true );
  data.autonomous = auton ? 1 : 0;
